...
---
Happy New Year! Excuse my absense while I have been away studying for GCFA and enjoying the holidays. Now that's all over I'm back to show you some static analysis of a macOS trojan I found today. Hope you enjoy and please send any questions, or comments to ragingroosterrem@gmail.com. Thanks!
...
### The Story, YAFFD
I was googling a security tool called moloch + threat hunting from a mac and clicked on a link to what I thought was a security blog containing some juciy information. It was the third link on the first page of results mind you! Instead of being served up some content to make my day easier, I was redirected to Yet Another Fake Flash Dropper (YAFFD). Okay, TBH I preffered to do some malware analysis anyway.

<img src="{{ site.baseurl }}/images/YAFFD.png">

### Analysis

#### After I clicked the pretty download button and received my shiny new flash installer (.dmg), I fired up my shell and went to my downloads directory to check this puppy out.
```
$ cd ~/Downloads
...
```
_Hey! Wouldn't Adobe sign their product is it was legit?_
...
```
_Looks like "Xl5NyWPPpAg.cD3QOlw2RV1GoEhnKg" is the binary executed by the .dmg at launch._
...
eval "$(_m "$_t" "$_y")"
```
_Gross! I don't want to spend all day on those loops. These functions are likely used to call down another binary, hence the term "dropper". The "t" variable is defintely obfuscated, likely multiple times based on the printf and xxd commands._
...
```
_There is a lot of output in the debug file, we may need to reference this later, but for now the important part is a file was downloaded from api[.]masteranalyser[.]com with some unique info from my machine, placed in the "tmp" directory, and then executed. We can see now that the variable "y" in the "enc" file above was actually Player.app. The "t" variable was likely used to enumerate the OS version and UUID of my machine which was used to form the URL. Note: I defanged the URL in the output above._
